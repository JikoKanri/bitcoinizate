(() => {
  ...
  function buyBtc() {
    ...
    S.btc += S.cash / S.price; S.cash = 0; S.buys++; A.sfx.buy();
  }
  function sellBtc() {
    if (S.phase !== "play" || S.btc <= 0) return;
    S.cash += S.btc * S.price; S.btc = 0; S.sells++; A.sfx.sell();
  }
  function buyVt() {
    if (S.phase !== "play" || S.level < 2 || S.cash <= 0 || S.vtPrice <= 0) return;
    S.vt += S.cash / S.vtPrice; S.cash = 0; S.buys++; A.sfx.buy();
  }
  function sellVt() {
    if (S.phase !== "play" || S.level < 2 || S.vt <= 0) return;
    S.cash += S.vt * S.vtPrice; S.vt = 0; S.sells++; A.sfx.sell();
  }
  ...
  function step(dt) {
    ...
    for (let i = S.pipes.length - 1; i >= 0; i--) {
      const p = S.pipes[i];
      p.x -= speed * dt;
      if (!p.scored && p.x + pw < S.bird.x) { p.scored = true; S.cash += 100; S.candles++; A.sfx.coin(); }
      ...
      if (inX) {
        ...
        if (S.bird.y - hitR < ends.top + 2 || S.bird.y + hitR > ends.bot - 2) {
          if (S.power === "BULL") { burst(p.x + pw * 0.5, S.bird.y, GREEN, 8); A.sfx.wave(); S.cash += 200; S.pipes.splice(i, 1); continue; }
